...
sample_app::StatusCode copyFromFloatToNative(float* floatBuffer,
    ...
    {
        __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "******************************************************************** input dims = %lu\n >>>>",dims[i]);
    }
    switch (QNN_TENSOR_GET_DATA_TYPE(tensor)) {
        case QNN_DATATYPE_UFIXED_POINT_8:
            __android_log_print(ANDROID_LOG_ERROR, "QNN_INF","ufp8\n");
            datautil::floatToTfN<uint8_t>(reinterpret_cast<uint8_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
                                          floatBuffer,
        ...
        case QNN_DATATYPE_UFIXED_POINT_16:
            ...
            datautil::floatToTfN<uint16_t>(reinterpret_cast<uint16_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
                                           floatBuffer,
                                           QNN_TENSOR_GET_QUANT_PARAMS(tensor).scaleOffsetEncoding.offset,
                                           QNN_TENSOR_GET_QUANT_PARAMS(tensor).scaleOffsetEncoding.scale,
                                           datautil::calculateElementCount(dims));
        ...
        case QNN_DATATYPE_INT_32:
            __android_log_print(ANDROID_LOG_ERROR, "QNN_INF","int32\n");
            if (datautil::StatusCode::SUCCESS !=
        ...
        default:
            ...
            __android_log_print(ANDROID_LOG_ERROR, "QNN_QNN", "copyFromFloatToNative -> Datatype not supported yet!\n");
            returnStatus = sample_app::StatusCode::FAILURE;
            break;
    }
    return returnStatus;
...
sample_app::StatusCode sample_app::QnnSampleApp::executeGraphs(float* input_buffer,cv::Mat& output_buffer,std::vector<size_t> &output_dims) {
    auto returnStatus = StatusCode::SUCCESS;
    __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "execute is running\n");
    for (size_t graphIdx = 0; graphIdx < m_graphsCount; graphIdx++) {
        ...
        if (iotensor::StatusCode::SUCCESS !=
            ...
            returnStatus = StatusCode::FAILURE;
            __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "input  fail\n");

            __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "graphs = %d\n",m_graphsCount);
            __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "input tensor version = %d\n",(*m_graphsInfo)[graphIdx].inputTensors->version);
        ...
        __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "inputCount = %d\n",inputCount);
        for (size_t inputIdx = 0; inputIdx < inputCount; inputIdx++) {
            if (m_inputDataType == iotensor::InputDataType::FLOAT &&
                ...
                __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "Inputs: %u QNN FLOAT32: %u\n",
                                    QNN_TENSOR_GET_DATA_TYPE(inputs), QNN_DATATYPE_FLOAT_32);

                std::vector<size_t> dims; //(1, 3, 128, 128)
                fillDims(dims, QNN_TENSOR_GET_DIMENSIONS(inputs), QNN_TENSOR_GET_RANK(inputs));

                __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "Dims: (%lu, %lu, %lu)\n",
                                    dims[0], dims[1], dims[2]);

                if (datautil::StatusCode::SUCCESS !=
                    datautil::castFromFloat<float>(
                            reinterpret_cast<float *>(QNN_TENSOR_GET_CLIENT_BUF(inputs).data),
                            input_buffer,
                            datautil::calculateElementCount(dims))) {
                    QNN_ERROR("failure in castFromFloat<bool>");
                    returnStatus = StatusCode::FAILURE;
...
//            LOGE("populateInputTensorFromFiles failed for input ");
        ...
        ///////////
        if (StatusCode::SUCCESS == returnStatus) {
            QNN_DEBUG("Successfully populated input tensors for graphIdx: %zu", graphIdx);
...
//            LOGE("Received nullptr");
                ...
                __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "Output from network!");
                ///////////////////////TODO

                Qnn_Tensor_t* output = &outputs[0];
                fillDims(output_dims, QNN_TENSOR_GET_DIMENSIONS(output), QNN_TENSOR_GET_RANK(output));

                // output_dims: (1, 3, 512, 512)
                __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "Output Dims: (%lu, %lu, %lu)\n",
...
//          returnStatus        = allocateBuffer<float>(&floatBuffer, elementCount);
                ...
                *buffer = (float*)malloc(elementCount * sizeof(float));
                if (nullptr == *buffer) {
                    __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "mem alloc failed for *buffer\n");
                    return StatusCode::FAILURE;
                }
                if (StatusCode::SUCCESS != returnStatus) {
                ...
                __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "OutPut data: %d", QNN_TENSOR_GET_DATA_TYPE(tensor));
                ...
                switch (QNN_TENSOR_GET_DATA_TYPE(tensor)) {
                    ...
                    case QNN_DATATYPE_FLOAT_32:
                        if (datautil::StatusCode::SUCCESS !=
                            datautil::castToFloat<float>(
                                    *out,
                                    reinterpret_cast<float*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
                                    elementCount)) {
                            QNN_ERROR("failure in castToFloat<bool>");
                            returnStatus = StatusCode::FAILURE;
                        }
                        ...
                        break;
                    default:
                        QNN_DEBUG("Datatype not supported yet!");
                        returnStatus = StatusCode::FAILURE;
                        break;
                }
                if (StatusCode::SUCCESS != returnStatus) {
...
//                    outputPaths, fileName, dims, QNN_DATATYPE_FLOAT_32, bufferToWrite, outputBatchSize))
                ...
                __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "after output length = %zu\n",length);


                //////////////////////////////
                for(int i=0;i<10;i++)
                {
                    __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "after output buffer[%d] = %f\n",i,bufferToWrite[i]);
                }
...
//                    for (int h = 0; h < H; ++h) {
//                        for (int w = 0; w < W; ++w) {
//                            cv::Vec3f& pixel = tmp.at<cv::Vec3f>(h, w);
//                            for (int c = 0; c < C; ++c) {
//                                pixel[c] = bufferToWrite[c * H * W + h * W + w];
//                            }
//                        }
//                    }
//                    output_buffer = tmp;
                    __android_log_print(ANDROID_LOG_ERROR, "QNN_INF", "Output Last Dims: (%lu, %lu, %lu)\n",
                                        output_dims[0], output_dims[1], output_dims[2]);
